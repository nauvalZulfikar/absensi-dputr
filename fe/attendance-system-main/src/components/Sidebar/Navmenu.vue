<template>
	<ul>
		<li
			...
			:key="i"
			:class="`${isHasSubMenu(item)} ${isActiveMenu(i)} ${isMenuActive(
				item
			)}`"
			class="single-sidebar-menu">
			<template v-if="isUserAllowed(item)">
...
export default {
    ...
    setup() {
        ...
        const isUserAllowed = (data) => {
			const dataUser = useUserStore();
			const user = dataUser?.user;
			if (user) {
                const check = user?.roles?.some((role) => {
                    return data?.allowedRoles.includes(role?.role?.name);
                })
				return check ?? true;
			}
		};
	...
	methods: {
		...
		submenuActiveClass(i) {
			return this.activeSubmenu === i
				? 'parent_active not-collapsed'
				: 'collapsed';
		},
		isNotChildAndNotHeader(item) {
			return !item?.child && !item?.isHeader && item?.link;
		},
		isHeaderWithoutChild(item) {
			return item?.isHeader && !item?.child && !item?.link;
		},
	...
	created() {
		...
		this.$store.userstore = useUserStore();
